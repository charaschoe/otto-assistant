...
const { saveTranscriptToObsidian } = require("./src/integrations/obsidian-writer");
...
const { exportToMarkdown, exportToPDF, exportToCSV } = require('./src/utils/exporters');
const { authenticateGoogle, uploadToGoogleDrive } = require('./src/integrations/google-drive');
...
rl.question("Enter the numbers of the options you want to use (e.g., 1,2): ", (answer) => {
  const selectedOptions = answer.split(',').map((opt) => opt.trim());

  rl.question("Do you want a transcript? (yes/no): ", (transcriptAnswer) => {
    const wantsTranscript = transcriptAnswer.toLowerCase() === 'yes';
    ...
    (async () => {
      ...
      if (wantsTranscript) {
        ...
        const python = spawn("python3", ["./src/transcription/whisper-transcribe.py", file]);
        ...
        python.on("close", (code) => {
          ...
          console.log(`⏱️ Transcription completed in ${duration} seconds.`);

          if (transcript.trim()) {
            if (selectedOptions.includes("1")) {
              console.log("✅ Exporting to Notion...");
              exportToNotion("Otto-Transkript", transcript.trim());
            }
            ...
            if (selectedOptions.includes("21")) {
              exportToMarkdown(transcript.trim(), "Otto-Transkript");
            }

            if (selectedOptions.includes("8")) {
              exportToPDF(transcript.trim(), "Otto-Transkript");
            }
            ...
            if (selectedOptions.includes("7")) {
              ...
              exportToCSV(structuredData, "Otto-Transkript");
            }

            if (selectedOptions.includes("12")) {
              (async () => {
                const auth = await authenticateGoogle();
                const filePath = path.join(__dirname, './exports/Otto-Transkript.md'); // Example file
                const fileName = 'Otto-Transkript.md';
                await uploadToGoogleDrive(auth, filePath, fileName);
              })();
            }
          } else {
        ...
        if (selectedOptions.includes("1")) {
          console.log("✅ Exporting to Notion without transcription...");
          exportToNotion("Otto-Transkript", "No transcription requested.");
        }

        if (selectedOptions.includes("2")) {
          console.log("✅ Exporting to Obsidian without transcription...");
          saveTranscriptToObsidian("No transcription requested.", "Otto-Transkript");
        }
      }
    })();
  });
