...
router.post("/credentials", authenticate, async (req, res) => {
	try {
		...
		if (!service || !data) {
			return res
				.status(400)
				.json({ error: "Service und Daten sind erforderlich" });
		}
		...
		if (success) {
			res.json({
				success: true,
				message: `${service}-Credentials erfolgreich gespeichert`,
			});
		} else {
			res.status(500).json({
		...
		res.status(500).json({
			error: "Server-Fehler beim Speichern der Credentials",
		});
	}
});
...
router.get("/credentials/:service", authenticate, async (req, res) => {
	try {
		...
		if (credential) {
			...
			res.status(404).json({
				error: `Keine ${service}-Credentials gefunden`,
			});
		}
	} catch (error) {
		console.error("Get Credentials Error:", error);
		res.status(500).json({
			error: "Server-Fehler beim Abrufen der Credentials",
		});
	}
});
