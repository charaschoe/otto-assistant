				...
				if (transcript.trim()) {
					...
					try {
						...
						if (outputOptions.includes("1")) {
							try {
								const notionCreds = await getCredential(
									userId,
									"notion"
								);
								...
								if (notionCreds) {
									...
									results.exportResults.notionPageId =
										notionResponse.data.id;
								} else {
						...
						if (outputOptions.includes("2")) {
							try {
								const obsidianCreds = await getCredential(
									userId,
									"obsidian"
								);
						...
						if (outputOptions.includes("12")) {
							try {
								const googleCreds = await getCredential(
									userId,
									"google"
								);
					...
					res.status(500).json({
						error: "Keine Transkription erstellt",
					});
				}
			});
		} catch (error) {
