...
import { saveTranscript, getTranscripts, getTranscript } from '../src/database/db-service.js';
import { transcribeWithWhisper, exportToNotion, exportToObsidian, generateWithGemini } from '../src/services/api-service.js';
...
router.post('/transcribe', authMiddleware, upload.single('audio'), async (req, res) => {
  try {
    ...
    try {
      ...
      const transcript = await transcribeWithWhisper(audioBuffer, userId);
      
      // Transkript in der Datenbank speichern
      const transcriptId = await saveTranscript(userId, transcript, title);
      ...
      if (outputOptions.includes('1')) {
        exportPromises.push(
          exportToNotion(title, transcript, userId)
            .then(pageId => { results.exportResults.notion = { status: 'success', pageId }; })
            .catch(error => { 
              console.error('Notion Export Error:', error);
              results.exportResults.notion = { status: 'error', message: error.message };
            })
        );
      }
      ...
      if (outputOptions.includes('2')) {
        exportPromises.push(
          exportToObsidian(title, transcript, userId)
            .then(() => { results.exportResults.obsidian = { status: 'success' }; })
            .catch(error => { 
              console.error('Obsidian Export Error:', error);
              results.exportResults.obsidian = { status: 'error', message: error.message };
            })
        );
      }
      ...
      if (outputOptions.includes('summary')) {
        exportPromises.push(
          generateWithGemini(`Fasse den folgenden Text kurz zusammen:\n\n${transcript}`, userId)
