...
export async function initTables() {
	...
	console.log(
		"Tabellen-Referenz bereitgestellt (manuelle Erstellung in Supabase UI erforderlich)"
	);
}
...
export async function saveCredential(userId, service, data) {
	try {
		...
		if (fetchError) {
			console.error(
				"Fehler beim Prüfen bestehender Credentials:",
				fetchError
			);
			return false;
		}
		...
		if (existingData) {
			...
			if (error) {
				console.error(
					"Fehler beim Aktualisieren von Credentials:",
					error
				);
				return false;
			}
