...
function generateTitle(transcript, defaultTitle = "Note") {
	...
	try {
		...
		// Prüfe, ob summary ein String ist
		if (typeof summary === "string") {
			return `${defaultTitle}: ${summary.substring(
				0,
				50
			)}... - ${formatDate(new Date())}`;
		}
		// Fallback: Nutze Keywords oder erste Wörter
		const keywords = extractKeywords(transcript);
		if (keywords.length > 0) {
			const topKeywords = keywords.slice(0, 3).join(", ");
			return `${defaultTitle}: ${topKeywords} - ${formatDate(
				new Date()
			)}`;
		}
